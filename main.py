...
def get_html_page_get_out_webbrowser():
    ...
    jump_to_the_right_section(driver, url=config.URL, login=config.LOGIN, password=config.PASSWORD)
...
def jump_to_the_right_section(driver, url, login, password):
    # Load page1
    driver.get(url=url)
    WebDriverWait(driver=driver, timeout=10).until(EC.presence_of_element_located((By.ID, "footer_utilities")))
    # Find and click login button
    driver.find_element(By.XPATH, '//*[@id="sign_in"]').click()
    time.sleep(2)
    # Fill login and pass
    driver.find_element(By.XPATH, '//*[@id="ips_username"]').send_keys(login)
    driver.find_element(By.XPATH, '//*[@id="ips_password"]').send_keys(password)
    driver.find_element(By.XPATH, '//*[@id="login"]/div[5]/input').click()
    WebDriverWait(driver=driver, timeout=10).until(EC.presence_of_element_located((By.ID, "footer_utilities")))
    # Go to section
    driver.find_element(By.XPATH, '//*[@id="category_36"]/div/div/div/div/table/tbody/tr[2]/td[2]/h4/strong/a').click()
    WebDriverWait(driver=driver, timeout=10).until(EC.presence_of_element_located((By.ID, "footer_utilities")))
    # Go to NOD32 topic
    driver.find_element(By.XPATH, '//*[@id="tid-link-47660"]').click()
    time.sleep(2)
    # Go to last page from topic
    driver.find_element(By.XPATH, '//*[@id="content"]/div[2]/div/ul[3]/li[2]').click()
